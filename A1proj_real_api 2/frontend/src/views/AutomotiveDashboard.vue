...
const gaugeSpindle = computed(() => ({
  ...
  series: [{ type: 'gauge', radius: '88%', center: ['50%', '55%'], startAngle: 210, endAngle: -30, min: 0, max: 100, splitNumber: 10, axisLine: { lineStyle: { width: 10, color: [[0.3, '#ff4d4f'], [0.7, '#faad14'], [1, '#52c41a']] } }, pointer: { length: '55%', width: 5, itemStyle: { color: '#00ffcc' } }, detail: { valueAnimation: true, formatter: '{value}%', color: '#fff', fontSize: 14, offsetCenter: [0, '62%'] }, data: [{ value: 72 }] }],
}))

const gaugeServo = computed(() => ({
  backgroundColor: 'transparent',
  series: [{ type: 'gauge', radius: '88%', center: ['50%', '55%'], startAngle: 210, endAngle: -30, min: 0, max: 50, splitNumber: 10, axisLine: { lineStyle: { width: 10, color: [[0.4, '#52c41a'], [0.7, '#faad14'], [1, '#ff4d4f']] } }, pointer: { length: '55%', width: 5, itemStyle: { color: '#faad14' } }, detail: { valueAnimation: true, formatter: '{value}μm', color: '#fff', fontSize: 14, offsetCenter: [0, '62%'] }, data: [{ value: 8 }] }],
}))
...
const efficiencyTrend = computed(() => ({
  ...
  xAxis: { type: 'category', data: ['0h','4h','8h','12h','16h','20h','24h'], axisLabel: { color: tc, fontSize: 8 }, axisLine: { lineStyle: { color: ac } } },
  yAxis: { type: 'value', name: '%', nameTextStyle: { color: tc, fontSize: 8 }, axisLabel: { color: tc, fontSize: 8 }, splitLine: { lineStyle: { color: '#0a2a3a', type: 'dashed' } } },
...
const toolBar = computed(() => ({
  ...
  xAxis: { type: 'category', data: ['T08','T09','T10','T11','T12','T13'], axisLabel: { color: tc, fontSize: 8 }, axisLine: { lineStyle: { color: ac } } },
  yAxis: { type: 'value', name: '%', nameTextStyle: { color: tc, fontSize: 8 }, axisLabel: { color: tc, fontSize: 8 }, splitLine: { lineStyle: { color: '#0a2a3a', type: 'dashed' } } },
...
const precisionTrend = computed(() => ({
  backgroundColor: 'transparent', tooltip: { trigger: 'axis', backgroundColor: '#0a1428', borderColor: '#0d4a7a', textStyle: { color: '#c0d0e0' } },
  legend: { data: ['加工精度', '粗糙度'], textStyle: { color: tc, fontSize: 9 }, top: 0 },
  grid: { left: '3%', right: '4%', top: '18%', bottom: '3%', containLabel: true },
  xAxis: { type: 'category', data: ['1月','2月','3月','4月','5月','6月'], axisLabel: { color: tc, fontSize: 8 }, axisLine: { lineStyle: { color: ac } } },
  yAxis: [{ type: 'value', name: 'μm', nameTextStyle: { color: tc, fontSize: 8 }, axisLabel: { color: tc, fontSize: 8 }, splitLine: { lineStyle: { color: '#0a2a3a', type: 'dashed' } } }, { type: 'value', name: 'Ra', nameTextStyle: { color: tc, fontSize: 8 }, axisLabel: { color: tc, fontSize: 8 }, splitLine: { show: false } }],
...
<style scoped>
.device-screen { width:100vw;height:100vh;font-family:'Microsoft YaHei','PingFang SC',monospace;color:#e0f0ff;overflow:hidden;display:flex;flex-direction:column;padding:0 20px 8px;box-sizing:border-box;position:relative;z-index:1; }
...
.tb-deco { color:#00ffcc;font-size:18px;filter:drop-shadow(0 0 10px #00ffcc); }
...
.metric { flex:1;text-align:center;padding:6px;background:rgba(0,255,204,0.04);border-radius:6px; }
.m-label { font-size:10px;color:#88bbaa; }
...
.sl-label { color:#88bbaa;flex:1; }
.sl-val { color:#d0e8e0; }

.center-stats { display:flex;gap:24px;justify-content:center;margin-top:8px; }
.cs-item { text-align:center; }
.cs-val { font-size:26px;font-weight:900;background:linear-gradient(180deg,#fff,#66bbff);-webkit-background-clip:text;-webkit-text-fill-color:transparent; }
.cs-unit { font-size:10px;color:#6aa090;margin-left:2px; }
.cs-lbl { display:block;font-size:10px;color:#4a7a8a; }

.chain { display:flex;align-items:center;gap:0;flex:1; }
.chain-node { text-align:center;flex:1; }
.cn-icon { font-size:22px; }
.cn-label { font-size:9px;color:#88bbaa;margin-top:2px; }
.chain-arrow { color:#1e4a7a;font-size:14px;flex-shrink:0; }
...
.al-time { color:#6aa090;flex-shrink:0;font-variant-numeric:tabular-nums; }
.al-msg { color:#d0e8e0; }
