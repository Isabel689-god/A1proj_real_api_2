...
const tc = '#88bbaa'; const ac = '#0d2a40'
...
const faultTypeRadar = computed(() => {
  ...
  return { backgroundColor:'transparent', tooltip:{backgroundColor:'#0a1428',borderColor:'#0d4a7a',textStyle:{color:'#c0d0e0'}},
    radar:{ center:['50%','52%'],radius:'62%', indicator:items.map(d=>({name:d.name,max:maxVal*1.2})), axisName:{color:tc,fontSize:9},
...
const tagBar = computed(() => ({
  backgroundColor:'transparent', tooltip:{trigger:'axis',backgroundColor:'#0a1428',borderColor:'#0d4a7a',textStyle:{color:'#c0d0e0'}}, grid:{left:'3%',right:'4%',top:'8%',bottom:'3%',containLabel:true},
  xAxis:{type:'category',data:tagDistribution.value.slice(0,8).map(d=>d.name),axisLabel:{color:tc,fontSize:9,rotate:30},axisLine:{lineStyle:{color:ac}}},
  yAxis:{type:'value',axisLabel:{color:tc,fontSize:9},splitLine:{lineStyle:{color:'#0a2a3a',type:'dashed'}}},
...
const graphOption = computed(() => ({
  backgroundColor:'transparent', tooltip:{backgroundColor:'#0a1428',borderColor:'#0d4a7a',textStyle:{color:'#c0d0e0'}},
  series:[{ type:'graph',layout:'force',roam:true,draggable:false, force:{repulsion:600,edgeLength:[50,180],gravity:0.04,friction:0.6}, edgeSymbol:['none','none'],
    ...
    label:{show:true,color:'#ffffff',fontSize:8,position:'right',formatter:(p:any)=>p.name.length>10?p.name.slice(0,10)+'…':p.name},
    emphasis:{focus:'adjacency',label:{show:true,color:'#fff',fontSize:11},itemStyle:{shadowBlur:25,shadowColor:'#00ffcc'}},
...
const pyramidOption = computed(() => ({
  backgroundColor:'transparent', tooltip:{trigger:'item',backgroundColor:'#0a1428',borderColor:'#0d4a7a',textStyle:{color:'#c0d0e0'}},
  series:[{ type:'pie',radius:['55%','82%'],center:['50%','52%'],
    itemStyle:{borderRadius:4,borderColor:'rgba(0,255,204,0.3)',borderWidth:1.5},
    label:{color:tc,fontSize:9,formatter:'{b}\n{d}%'},
    emphasis:{scaleSize:10,shadowBlur:25,shadowColor:'rgba(0,255,204,0.4)',label:{fontSize:13,fontWeight:'bold',color:'#fff'}},
    data:[
  ...
  graphic:[{type:'text',left:'center',top:'42%',style:{text:'85条',textAlign:'center',fill:'#d0f0e8',fontSize:18,fontWeight:'bold'}},{type:'text',left:'center',top:'52%',style:{text:'故障总计',textAlign:'center',fill:'#88bbaa',fontSize:11}}],
}))

const trendLine = computed(() => ({
  backgroundColor:'transparent', tooltip:{trigger:'axis',backgroundColor:'#0a1428',borderColor:'#0d4a7a',textStyle:{color:'#c0d0e0'}}, grid:{left:'3%',right:'4%',top:'8%',bottom:'3%',containLabel:true},
  xAxis:{type:'category',data:['周一','周二','周三','周四','周五','周六','周日'],axisLabel:{color:tc,fontSize:9},axisLine:{lineStyle:{color:ac}}},
  yAxis:{type:'value',axisLabel:{color:tc,fontSize:9},splitLine:{lineStyle:{color:'#0a2a3a',type:'dashed'}}},
...
<style scoped>
.big-screen { width:100vw;height:100vh;font-family:'Microsoft YaHei','PingFang SC',monospace;color:#e0f0ff;overflow:hidden;display:flex;flex-direction:column;padding:0 20px 8px;box-sizing:border-box;position:relative; }

/* ═══ 顶部 ═══ */
.top-bar { display:flex;align-items:center;justify-content:space-between;height:52px;flex-shrink:0;z-index:2;background:linear-gradient(180deg,rgba(0,80,100,0.35),transparent);border-bottom:1px solid rgba(0,255,204,0.2);padding:0 20px;position:relative; }
.corner-tl,.corner-tr,.corner-bl,.corner-br { position:absolute;width:16px;height:16px;border-color:#00ffcc;border-style:solid;opacity:.7; }
.corner-tl { top:8px;left:8px;border-width:2px 0 0 2px; }
.corner-tr { top:8px;right:8px;border-width:2px 2px 0 0; }
.corner-bl { bottom:8px;left:8px;border-width:0 0 2px 2px; }
.corner-br { bottom:8px;right:8px;border-width:0 2px 2px 0; }
.tb-left { display:flex;align-items:center;gap:8px; }
.tb-deco { color:#00ffcc;font-size:18px;animation:spin 6s linear infinite;filter:drop-shadow(0 0 10px #00ffcc); }
@keyframes spin { to{transform:rotate(360deg)} }
.tb-title { font-size:22px;font-weight:800;background:linear-gradient(90deg,#00ffcc,#fff,#00ffcc);-webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:5px;margin:0; }
.tb-right { display:flex;align-items:center;gap:16px; }
.tb-tag { font-size:11px;color:#00ffcc;border:1px solid rgba(0,255,204,.5);border-radius:12px;padding:2px 14px;text-shadow:0 0 8px rgba(0,255,204,.5); }
.tb-time { font-size:14px;color:#a0e8d0;font-variant-numeric:tabular-nums; }
...
/* ═══ 模块框 ═══ */
.box { background:linear-gradient(135deg,rgba(3,15,35,0.55),rgba(5,22,48,0.6));border:1px solid rgba(0,255,204,0.22);border-radius:8px;padding:16px;backdrop-filter:blur(12px);box-shadow:0 0 16px rgba(0,255,204,0.1),0 0 32px rgba(0,255,204,0.04),inset 0 0 16px rgba(0,255,204,0.04);position:relative;overflow:hidden;display:flex;flex-direction:column; }
.box::before { content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(0,255,204,0.5),transparent);animation:sweep 3s ease-in-out infinite; }
@keyframes sweep { 0%,100%{opacity:.15;transform:scaleX(.1)} 50%{opacity:1;transform:scaleX(1)} }
.box-head { font-size:13px;font-weight:700;color:#d0f0e8;margin-bottom:10px;display:flex;align-items:center;gap:6px;flex-shrink:0; }
.bh-diamond { color:#00ffcc;font-size:12px;text-shadow:0 0 6px #00ffcc; }
.legend-row { display:flex;align-items:center;gap:5px;margin-left:auto;font-size:10px;color:#88bbaa;font-weight:400; }
...
.mini-card { background:linear-gradient(135deg,rgba(0,255,204,0.06),rgba(0,100,120,0.03));border:1px solid rgba(0,255,204,0.18);border-radius:6px;padding:12px 8px;text-align:center;transition:transform .3s,border-color .3s,box-shadow .3s;display:flex;flex-direction:column;justify-content:center;box-shadow:0 0 8px rgba(0,255,204,0.06); }
.mini-card:hover { transform:translateY(-2px);border-color:rgba(0,255,204,.45);box-shadow:0 0 18px rgba(0,255,204,.2); }
.mc-val { font-size:26px;font-weight:900;font-variant-numeric:tabular-nums; }
.mc-icon { font-size:18px;margin:2px 0; }
.mc-label { font-size:11px;color:#88bbaa; }

/* ═══ 动态表格 ═══ */
.dyn-table { font-size:11px;flex:1;overflow:auto; }
.dt-hdr { display:flex;gap:8px;color:#6aa090;border-bottom:1px solid rgba(0,255,204,.12);padding-bottom:6px;margin-bottom:4px;font-weight:600; }
.dt-hdr span:first-child { width:42px; }
.dt-hdr span:nth-child(2) { flex:1; }
.dt-hdr span:last-child { width:46px; }
.dt-row { display:flex;gap:8px;padding:4px 0;align-items:center;border-bottom:1px solid rgba(0,255,204,.04); }
.dtr-time { width:42px;color:#6aa090;flex-shrink:0;font-variant-numeric:tabular-nums; }
.dtr-msg { flex:1;color:#d0e8e0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }
